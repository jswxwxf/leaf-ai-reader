...
- 保留 Markdown 的语义结构，尤其是标题、链接、列表、表格、引用、代码块和图片。
...
Markdown 生成的 HTML 已有稳定结构，不应送入完整 `cleanHtml` 管线。当前管线包含 DOM 展平、标签剥离、来源兼容修复和排版重组，可能破坏 Markdown 的表格、链接、代码块及标题层级。
...
链接文字可作为其所属文本块的一部分参与分句，但必须保留 `<a>` 结构与 `href`，不能因分句破坏链接。
...
- 保留：段落、标题、强调、删除线、列表、引用、表格、代码块、链接、图片、换行及 sentence span。
- 移除：脚本、事件属性、内联样式、iframe、表单、嵌入式媒体及其他主动内容。
- 限制链接与图片 URL 协议，拒绝危险协议。
- 保留链接必要属性（至少 `href`）；外链打开策略与现有阅读器安全策略保持一致。
...
- [ ] 覆盖标题、列表、链接、引用、表格、代码块、图片与原始 HTML 的渲染回归测试。
...
4. Markdown 标题、强调、删除线、列表、引用、表格、代码块、链接和图片的结构得到保留。
5. 恶意脚本、事件属性与危险 URL 协议不会出现在最终 `content.html` 中。
