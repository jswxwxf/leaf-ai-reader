# PRD-017: 通用型 SpeechBox 朗读控制台组件设计与实现方案

**状态**：已完成首版

**实现范围**：
- 已新增通用 `SpeechBox` 组件，可包裹任意包含文字的 UI。
- 已在阅读器摘要卡片中接入，用右上角小喇叭按钮触发朗读。
- 已支持通过根节点 `innerText` 自动提取文本，无需额外传入 `text`。
- 已实现点击朗读前调用 `window.speechSynthesis.cancel()`，保证同一时间只有一个声音播放。
- 暂未实现词级高亮、播放态样式和暂停/继续控制，后续如有需要可继续扩展。
...
## 5. 验收标准
1.  `SpeechBox` 可直接包裹摘要卡片等现有 UI，不要求调用方传递冗余文本字段。
2.  点击小喇叭后可朗读包裹区域内的文本内容。
3.  朗读过程中点击另一个 `SpeechBox`，会先取消当前朗读再播放新的内容。
4.  小喇叭按钮不会触发摘要卡片原有的点击跳转行为。

## 6. 后续增强
1.  使用 CSS Custom Highlight API 实现朗读过程中的词级高亮。
2.  增加播放态视觉反馈，例如当前按钮高亮或显示停止图标。
3.  增加暂停/继续等更完整的碎片朗读控制能力。
